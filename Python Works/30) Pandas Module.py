...
print(dfh,dfb)
#you can also use .imdb_rating.min()/max()/mean() and .rows or .columns and .industry.unique() for knowing rows and columns we can df.shape, df.rows and df.columns.
...
print(g.size())
print(g.describe())
#pd.concat([list dfs],keys=[specify keys],ignore_index=True,axis=1 forcolmerging 0 forrowmerging)
